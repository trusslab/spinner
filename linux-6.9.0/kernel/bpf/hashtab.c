				   ...
				   unsigned long *pflags)
{
	...
	if (in_nmi()) {
		if (!raw_spin_trylock(&b->raw_lock)) {
			__this_cpu_dec(*(htab->map_locked[hash]));
                	local_irq_restore(flags);
                	preempt_enable();
			return -EBUSY;
...
static inline void htab_unlock_bucket(const struct bpf_htab *htab,
				      ...
				      unsigned long flags)
{
        //s2e_concretize(htab, sizeof(struct bpf_htab));
        //s2e_concretize(b, sizeof(struct bucket));
        //s2e_concretize(&hash, sizeof(u32));
        //s2e_concretize(flags, sizeof(unsigned long));
...
static long htab_map_update_elem(struct bpf_map *map, void *key, void *value,
				 u64 map_flags)
{
	s2e_make_symbolic(key, sizeof(int), "key");
        s2e_make_symbolic(value, sizeof(int), "value");
        s2e_make_symbolic(&map->max_entries, sizeof(map->max_entries), "max_entries");
        s2e_make_symbolic(&map->map_flags, sizeof(map->map_flags), "map_flags");
